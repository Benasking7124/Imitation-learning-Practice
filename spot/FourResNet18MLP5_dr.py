...
class FourResNet18MLP5_dr(nn.Module):
    def __init__(self):
        super(FourResNet18MLP5_dr, self).__init__()
        ...
        self.fc_layer2 = nn.Sequential(
            nn.Linear(1024, 256),
            nn.ReLU())
        self.transform2 = nn.Linear(1024, 256)

        self.fc_layer3 = nn.Sequential(
            nn.Linear(256, 64),
            nn.ReLU())
        self.transform3 = nn.Linear(256, 64)
        
        self.fc_layer4 = nn.Sequential(
            nn.Linear(64, 16),
            nn.ReLU())
        self.transform4 = nn.Linear(64, 16)

        self.fc_layer5 = nn.Linear(16, 1)
    ...
    def forward(self, image1, image2, image3, image4):
        ...
        output2 = self.fc_layer2(output1) + self.transform2(output1)
        output3 = self.fc_layer3(output2) + self.transform3(output2)
        output4 = self.fc_layer4(output3) + self.transform4(output3)
